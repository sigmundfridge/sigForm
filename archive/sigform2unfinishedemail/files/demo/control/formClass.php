...
class mailForm extends formController {
	...
	function sendMail($values) {	
		...
		if (mail($to, $values['messageSubject'], $message, $headers)){
			...
			echo  '<p>If you\'d like to send another message, please go to <a href="contact.php">our contact page</a></p>';
			echo $siteEmail.' and '.$newEmail;
			foreach($values as $key=>$value) echo '<p>'.$key.'<br>'.$value.'</p>';
		}
		else {
